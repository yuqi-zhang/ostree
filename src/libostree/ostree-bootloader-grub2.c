  ...
  for (i = 0; i < loader_configs->len; i++)
    {
      ...
      if (!g_data_output_stream_put_string (dataout, hardcoded_insmods, cancellable, error))
        goto out;
      /* Bit extracted from the helper shell script library */
      if (!g_data_output_stream_put_string (dataout, grub2_prepare_root_cache, cancellable, error))
        goto out;
      ...
      if (options)
        {
          if (!g_data_output_stream_put_string (dataout, " ", cancellable, error))
            goto out;
